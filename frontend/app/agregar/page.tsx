...
export default function CreatePinLocation() {
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-6 pb-24">
      ...
      <p className="text-sm text-muted-foreground mb-6">Ayudá a la comunidad celíaca marcando un local apto</p>

      <div className="flex flex-col gap-4">
        <div>
          <label className="text-xs font-semibold text-foreground uppercase tracking-wider mb-1 block">Nombre</label>
          <input
            type="text"
        ...
        <div>
          <label className="text-xs font-semibold text-foreground uppercase tracking-wider mb-1 block">Ubicación</label>
          <p className="text-xs text-muted-foreground mb-2">Tocá el mapa para marcar la ubicación</p>
          <div className="rounded-2xl overflow-hidden border border-border h-[300px]">
            <LocationPicker onSelect={setCoords} />
          </div>
          {coords && (
            <p className="text-xs text-primary mt-1">✓ Ubicación seleccionada</p>
          )}
        </div>
